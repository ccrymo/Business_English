...
import examRevision from "../../data/examPrep/examPrepSlides";
...
const TitleSlide = React.memo(({ title }) => (
  <div className="flex items-center justify-center h-[calc(100vh-200px)]">
    <motion.h2
      ...
      className="text-4xl md:text-6xl lg:text-6xl font-bold text-red-500 text-center px-6"
    >
        ...
        if (Array.isArray(value)) {
          return (
            <motion.div
              ...
              key={key}
            >
              <h3 className="text-2xl lg:text-4xl font-bold mb-8 text-red-500">
                ...
                {value.map((item, index) => (
                  ...
                  >
                    {typeof item === "object" ? (
                      <div className="mb-3">
                        {item.topic && (
                          <h4 className="text-red-500 font-extrabold mb-2">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-neutral-900/80 backdrop-blur-sm">
        <div className="max-w-4xl mx-auto flex gap-4">
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="flex-1 py-3 px-6 rounded-lg font-bold text-white hover:text-red-950 bg-neutral-800 hover:bg-red-500 transition-colors"
            onClick={handlePrev}
          >
            Previous
          </motion.button>
          <motion.button
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className={`flex-1 py-3 px-6 rounded-lg font-bold text-white hover:text-red-950 ${
              isLastSlide ? "bg-neutral-600 cursor-not-allowed" : "bg-neutral-800 hover:bg-red-500"
            } transition-colors`}
            onClick={handleNext}
